...
def main():
    ...
    timer.counter()                         # gets the timer value
...
def timer_int(tim_num):
    ...
    #COLLECT ADC
    int_queue.put()  #read and put into queue. inside the put() is the value that will be read from the pin. this has not been set up in the code yet
    int_queue.full()
...
def step_response():# run this function when requeste dby user or through GUI
      ...
      """
      # Function code here
time_int.callback (timer_int)  #configure and enable the calllback. example : timmy.callback(timer_cb)
